                ...
                if let Some(libraries) = forge_json["libraries"].as_array() {
                    for lib in libraries {
                        ...
                        if parts.len() >= 3 {
                            ...
                            if parts.len() >= 4 {
                                jar_name = format!("{}-{}-{}.jar", artifact, lib_version, parts[3]);
                            }
                            let path = mc_dir.join("libraries").join(&group).join(artifact).join(lib_version).join(&jar_name);
                            cp_entries.push(path.to_string_lossy().into_owned());
                        }
...
pub async fn download_forge(mc_version: &str, forge_version: &str, mc_dir: &PathBuf, java_path: &PathBuf, app: &AppHandle) -> Result<(), String> {
    ...
    let _ = app.emit("download_progress", ProgressPayload {
        task: format!("Downloading Forge Installer"),
        progress: 0,
    });
    ...
    if !res.status().is_success() {
        ...
        let total_size = res_alt.content_length().unwrap_or(0);
        let mut file = fs::File::create(&temp_installer).map_err(|e| e.to_string())?;
        let mut stream = res_alt.bytes_stream();
        let mut downloaded: u64 = 0;
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| e.to_string())?;
            use std::io::Write;
    ...
    let _ = app.emit("download_progress", ProgressPayload {
        task: format!("Running Forge Installer (this may take a while)"),
        progress: 100,
    });
    
    let profiles_path = std::path::Path::new(mc_dir).join("launcher_profiles.json");
