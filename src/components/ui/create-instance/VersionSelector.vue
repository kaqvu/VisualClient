...
const filteredVersions = computed(() => {
  ...
  return versions.value.filter(ver => 
    ver.toLowerCase().includes(versionSearch.value.toLowerCase())
  );
});
...
onMounted(async () => {
  ...
  try {
    ...
    versions.value = relVersions.map(v => v.id);
    relVersions.forEach(v => {
      versionUrls.value[v.id] = v.url;
    });
      
    if (data.latest && data.latest.release) {
      versionSearch.value = data.latest.release;
      emit('update:modelValue', data.latest.release);
      emit('update:url', versionUrls.value[data.latest.release] || '');
    } else if (versions.value.length > 0) {
      versionSearch.value = versions.value[0];
      emit('update:modelValue', versions.value[0]);
      emit('update:url', versionUrls.value[versions.value[0]] || '');
    }
  } catch (e) {
...
const selectVersion = (v: string) => {
  versionSearch.value = v;
  isDropdownOpen.value = false;
  emit('update:modelValue', v);
  emit('update:url', versionUrls.value[v] || '');
};
</script>
        ...
        <div class="versions-list">
          ...
          <template v-else>
            <div 
              ...
              :key="v" 
              class="version-card"
              :class="{ selected: versionSearch === v }"
